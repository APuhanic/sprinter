	...
	<div class="mx-auto max-w-7xl px-4">
		...
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
			...
			<div class="lg:col-span-5">
				...
				<div class="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs text-slate-600">
					<div class="flex items-center gap-1.5 bg-slate-50 rounded px-2 py-2">
						<svg class="w-3.5 h-3.5 text-slate-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
				...
				<div class="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs text-slate-600">
					...
					<div class="flex items-center gap-1.5 bg-slate-50 rounded px-2 py-2">
						<svg class="w-3.5 h-3.5 {vehicle.hasAC ? 'text-brand-green' : 'text-slate-300'} shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
						<span>{t.rental.climate}: {vehicle.hasAC ? t.rental.yes : t.rental.no}</span>
					</div>
					<div class="flex items-center gap-1.5 bg-slate-50 rounded px-2 py-2">
						<svg class="w-3.5 h-3.5 {vehicle.hasCruise ? 'text-brand-green' : 'text-slate-300'} shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
						<span>{t.rental.cruise}: {vehicle.hasCruise ? t.rental.yes : t.rental.no}</span>
					</div>
				</div>
			</div>

			<!-- Title + Description (right, 7 cols) -->
			<div class="lg:col-span-7">
				<h2 class="text-2xl font-bold text-slate-900 mb-2">{vehicle.name}</h2>
				...
				{#if vehicle.speedLimit}
					<div class="mb-6 flex items-start gap-2 text-sm text-amber-700 bg-amber-50 px-3 py-2.5 rounded">
		...
		<div class="mt-12">
			<h3 class="text-xl font-bold text-slate-900 mb-4">{t.rental.pricingTitle}</h3>
		...
		<div class="mt-10 text-center">
			...
			<a
				...
				class="inline-block border-2 border-brand-red text-brand-red px-8 py-2.5 rounded font-medium text-sm hover:bg-brand-red hover:text-white transition-colors"
			>
				{t.rental.viewTerms}
			</a>
		</div>

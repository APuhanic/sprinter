...
<section class="bg-white border-b border-slate-200">
	<div class="mx-auto max-w-7xl px-4 py-4">
		...
		<nav class="flex items-center gap-2 text-sm text-slate-500">
			...
			<span>&gt;</span>
			<a href="/{lang}/{slugs[lang].rental}" class="hover:text-brand-red transition-colors"
				>{t.rental.breadcrumb}</a
			>
			<span>&gt;</span>
		...
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
			...
			<div class="lg:col-span-5">
				...
				<div class="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs text-slate-600">
					...
					<div class="flex items-center gap-1.5 bg-slate-50 rounded px-2 py-2">
						<svg
							class="w-3.5 h-3.5 {vehicle.hasAC ? 'text-brand-green' : 'text-slate-300'} shrink-0"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
							><path
					...
					<div class="flex items-center gap-1.5 bg-slate-50 rounded px-2 py-2">
						<svg
							class="w-3.5 h-3.5 {vehicle.hasCruise
								? 'text-brand-green'
			...
			<div class="lg:col-span-7">
				...
				{#if vehicle.speedLimit}
					<div
						class="mb-6 flex items-start gap-2 text-sm text-amber-700 bg-amber-50 px-3 py-2.5 rounded"
					>
						<svg
						...
						{#each vehicle.pricing as row, i}
							<tr class="{i % 2 === 0 ? 'bg-white' : 'bg-slate-50'} border-b border-slate-100">
								<td class="px-4 py-3 text-slate-700 font-medium">
									{row.days}
									{row.days === 1 ? t.rental.day : t.rental.days}
								</td>
		...
		<div class="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6">
			...
			<div class="border border-slate-200 rounded-lg p-6">
				...
				<ul class="space-y-2 text-sm text-slate-600">
					...
					<li class="flex items-start gap-2">
						<svg
							class="w-4 h-4 text-brand-red shrink-0 mt-0.5"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
							><path
